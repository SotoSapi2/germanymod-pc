...
std::unordered_map<void*, PatchInfo> patches;
...
std::optional<Memory::ModuleInfo> Memory::FindModule(const char* moduleName)
{
	...
	for (auto module : modules)
	{
		...
		if (strcmp(module_filename, moduleName) == 0)
		{
			...
			GetModuleInformation(process_handle, module, &moduleInfo, sizeof(MODULEINFO));
...
std::unique_ptr<BYTE[]> Memory::ReadBytes(void* address, size_t len)
{
	std::unique_ptr<BYTE[]> data = std::make_unique<BYTE[]>(len);
	ReadProcessMemory(GetCurrentProcess(), address, data.get(), len * sizeof(BYTE), nullptr);
